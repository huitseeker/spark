...
class LatestSpeedListener(batchDuration: Duration) extends StreamingListener {
  ...
  val proportionalK: Double = 1
  val integralK: Double = - 0.01
  ...
  override def onBatchCompleted(batchCompleted: StreamingListenerBatchCompleted){
    this.synchronized{
      ...
      if (latestTime.isEmpty || newTime > latestTime.get && workDelay.isDefined){
        ...
        // Percentage of a batch interval this job scheduled for
        val waitRatio = waitDelay.getOrElse(0L).toDouble / batchDuration.milliseconds
        ...
        streamIdToElemsPerBatch = Some(elements.mapValues{ elements =>
          ...
          // at that speed during one batch interval, if the wait time had
          // been compute time (i.e. given an amount of time equal to the
          // scheduling delay),
          ...
          // component of a PID.
          val waitElementsThisBatch = waitRatio * elementsThisBatch

          val batchElements = proportionalK * elementsThisBatch + integralK * waitElementsThisBatch
          math.floor(batchElements).toLong
        })
      }
    }
  }
